...
{    
	...
	TRISB   |= 0x01; // Set B0 as input
	ADCON1   = 0x0F; // Turn off the A/D converter entirely
	INTCON2 |= 0x80; // Force the Port B pull up off.
	INTCON2 &= 0b10111111; // Interrupt on INT0 falling edge
	INTCON  |= 0b11010000; // Enable all interrupts. PIE on. INT0 on.
	
	// Set up the reset timer
	T2CON = 0b01111011; // Timer off. Prescale 16. Postscale 16.
	TMR2 = 0;           // Reset timer
	PR2 = 200;          // Match register
